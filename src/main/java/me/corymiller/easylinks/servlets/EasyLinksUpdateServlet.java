...
public class EasyLinksUpdateServlet extends HttpServlet {
        ...
        public void doGet(HttpServletRequest request, HttpServletResponse response)
                ...
                if (rl.size() == 0) {
                        response.sendError(HttpServletResponse.SC_BAD_REQUEST, "link does not exist");
                } else {
                        PreparedStatement updatePs = session
                                        .prepare("UPDATE permlinks " + "SET destination=? WHERE source=?");
                        BoundStatement updateBs = updatePs.bind(params.get("dest")[0], params.get("source")[0]);
